...
- [x] **T017B 受限同源音频中转**
  ...
  目标：将已获上游授权但为 HTTP 的短期音源转换为会话绑定的同源实时流，使浏览器不接触原始 URL，同时恢复公开精选与播放链路的可用性。

  允许修改：当前执行卡列出的文档、Provider 音源规范化、Session 内存登记、source/audio Route Handler、Player source 接入及对应测试。

  不允许破坏：不创建公开代理、第三方匹配、解灰、下载、转码、磁盘/持久缓存或部署能力；不暴露上游 URL、Cookie、代理、Location 或原始媒体响应；不改变唯一 Audio、队列边界、一次刷新和 Demo/Real 规则。

  页面测试：已发布 source 的 loading/playing/拖动；source 缺失或过期、非法 Range、上游 403/超时、媒体错误后的刷新、首页候选恢复、搜索播放以及 1440/768/390。

  验收：同源 Relay 的 Range/重定向/响应头/会话隔离契约通过；HTTP 实测只记录聚合 `206` 证据；浏览器与日志均无真实 URL；完整测试、`npm run check`、敏感信息扫描与 `git diff --check` 通过。

  完成记录：新增会话绑定的同源音频中转；`/api/tracks/[id]/source` 只返回本地 `/api/tracks/[id]/audio` 路径。Relay 校验 HTTP/HTTPS 网易云 CDN 主机、无凭据 URL、单一 Range 和最多三次允许重定向，仅透传媒体允许响应头并始终 `no-store`；会话变更或过期会清除已登记音源。真实本地探测记录为 `206`。验证通过：unit 12 文件/73 项、contract 7 文件/48 项、component 11 文件/61 项、Playwright 34 项、Canvas 非空检查 3/3、`npm run check`、敏感信息扫描与 `git diff --check`。`lint` 保留既有两条 `<img>` 优化 warning，无 error。

- [x] **T017 专辑、歌手、新歌与热门推荐**
...
- [x] **T018P 性能治理文档与计划对齐**
  ...
  目标：基于实际代码审查，将首页画廊的永久 rAF、指针悬停时逐帧 raycast，以及播放器可见播放态逐帧 `MEDIA_TIME` 发布列为可验证的性能治理对象；先统一产品、视觉、架构、状态机和执行计划，再进入任何运行时代码修改。

  允许修改：`TODO.md`、`docs/research/ECHOFORM_PRD_DRAFT.md`、`docs/research/ECHOFORM_VISUAL_DESIGN.md`、`docs/research/TECHNICAL_ARCHITECTURE.md`、`docs/research/PLAYER_STATE_MACHINE.md`。

  不允许修改：任何 `src/**`、测试、BFF/API/Provider、Session、音频与队列契约、依赖、`.env*`、CI/CD、部署和外部 URL；不删除、不推送。

  不允许破坏：既有有限画廊、波浪、预览/退出、头像共享转场、唯一 Audio、歌词时间源、后台播放、DOM fallback、Reduced Motion 与已通过的 T001-T018R 行为。

  验收：五份控制文档给出相同的调度状态、质量层级、纹理驻留、指针 raycast、高频时间订阅和量化验收定义；`T018S` 的允许范围、保护项、状态矩阵与证据方式可直接执行；`git diff --check`、敏感信息扫描与 `npm.cmd run check` 通过。
...
- [x] **T018S 画廊与播放器性能治理**
  ...
  允许修改：当前执行卡列出的文件与测试；开始编码前先补齐或更新 `FilmstripGallery.spec.md` 与 `PersistentPlayer.spec.md` 的实现级约束。

  不允许破坏：当前执行卡的全部保护项；不以降低动画质量、延迟歌词、跳过状态更新、关闭无障碍或仅在开发模式调参来伪造性能改善。
