...
- 不允许破坏：不使用全屏圆形遮罩；不展示手机号、生日、城市等非必要字段；不伪造品味画像；他人页面不显示编辑入口；头像错误使用中性首字母占位；所有入口为本地路由。
- 验收标准：头像共享元素约 600ms 且可中断；首屏信息层级正确；loading、自己的主页、公开他人主页、空资料/空歌单、私密/无权限、404、接口错误、头像失败与未登录访问均在 1440/768/390、焦点和错误恢复下通过；专项 tests 与 `npm run check` 通过。
