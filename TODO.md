...
- 不允许破坏：播放器单实例、有限队列、会话边界、现有本地路由、首页有限画廊与 Demo/Real 显式区分。
- 验收标准：IndexedDB schema 有版本与 adapter 测试；音频时间事件不会导致高频重复写入；列表末项不被播放器/导航遮挡；未登录、loading、各 Tab 正常/空、接口错误、离线历史、重复播放、阈值前后、清空确认/取消/失败与 1440/768/390 页面测试通过；专项 tests 与 `npm run check` 通过。
