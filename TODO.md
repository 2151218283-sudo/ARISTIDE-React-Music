...
- 当前任务：`T017 专辑、歌手、新歌与热门推荐`（未开始）
- 状态：T017A 与 T017B 已完成验收；T017 尚未获开发批准。
- 修改目标：完成 `/album/[id]`、`/artist/[id]`，以及搜索空状态和发现次级入口所需的新歌与热门数据；支持播放全部和本地详情导航。
- 允许修改：开始 T017 前，按其任务条目与控制文档重新确认允许路径和组件规格。
- 不允许修改：在未获得新的开发批准前不修改源代码、控制文档、环境、部署、数据库或 Git 历史。
- 不允许破坏：首页 WebGL 仅承载每日推荐、常规详情页使用纵向滚动、不可播放歌曲仍可查看但不伪造可播，以及所有站内导航保持本地路由。
- 验收标准：开始 T017 前按该任务条目、产品文档和页面状态矩阵重新制定并执行。
